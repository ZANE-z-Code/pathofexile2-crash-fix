...
print("==========================================")
print("Enhanced POE CPU Affinity Script - Stabilized")
print("==========================================")
time.sleep(1)

# Configuration parameters
POSSIBLE_PROCESSES = ["PathOfExileSteam.exe", "PathOfExile.exe", "PathOfExile_x64.exe"]
DISK_USAGE_THRESHOLD_MB = 5  # Threshold for high disk IO activity in MB/s
MEMORY_USAGE_THRESHOLD_MB = 300  # Memory increase threshold in MB
THREAD_COUNT_THRESHOLD = 5  # Number of new threads spawned during loading
CHECK_INTERVAL = 0.25  # Check interval in seconds
HYSTERESIS_THRESHOLD = 8  # Seconds of sustained low activity before restoring affinity
ACTIVITY_WINDOW = 4  # Number of checks for sustained activity tracking
...
logging.info("CPU masks prepared. Physical mask: %s, Full mask: %s", bin(physical_mask), bin(full_mask))
...
def monitor_process():
    ...
    while True:
        ...
        logging.info("Path of Exile process detected. Monitoring...")
        ...
        try:
            ...
            logging.info(
                "Resource Check: Avg Disk Usage=%.2f MB/s, Avg Memory Change=%.2f MB, Thread Count Change=%d",
                avg_disk_usage,
                avg_memory_change,
                thread_count_change,
            )
            ...
            if avg_disk_usage >= DISK_USAGE_THRESHOLD_MB or avg_memory_change > MEMORY_USAGE_THRESHOLD_MB or thread_count_change > THREAD_COUNT_THRESHOLD:
                if not is_smt_disabled:
                    logging.info(
                        "Map transition detected! Disk=%.2f MB/s, Memory Change=%.2f MB, Threads Change=%d",
                        avg_disk_usage,
                        avg_memory_change,
                        thread_count_change,
                    )
                    logging.info("Adjusting CPU affinity for high activity...")
                ...
                if is_smt_disabled:
                    if not under_threshold_start:
                        ...
                        logging.info("Sustained low activity. Restoring CPU affinity...")
                        set_cpu_affinity(process, full_mask)
                        is_smt_disabled = False

        except psutil.NoSuchProcess:
            logging.info("Process ended. Waiting for restart...")
            previous_io_counters = None
            previous_memory_usage = None
            previous_thread_count = None
            time.sleep(5)
